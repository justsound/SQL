DROP DATABASE IF EXISTS hw_4dop;
CREATE DATABASE hw_4dop;
USE hw_4dop;
...
SELECT 
	 u.firstname AS 'Имя пользователя',
     u.lastname AS 'Фамилия пользователя'
FROM users u
...
FROM users u
JOIN messages m
ON m.from_user_id = u.id AND m.to_user_id = 3
